...
from TkApplication.toolpg import configPage
from TkApplication.mods.bin_mod import bin_mod
import importlib
...
class Mainpg(object):
    ...
    def __init__(self, root):
        ...
        rootsize = '%dx%d+%d+%d' % (
        285, 480, (self.root.winfo_screenwidth() - 285) / 2, (self.root.winfo_screenheight() - 480) / 2)
    ...
    def phonepg(self):
        ...
        self.more_button = ttk.Button(self.phonebn_Frame3, text='微信缓存清空', width=26, command=self.more_bn_f).pack(
...
from TkApplication.mods import GetPhone
import time
import datetime
import logging
from .mainpg import Mainpg
from TkApplication.mods.bin_mod import *
from TkApplication.mods import ThreadClass
from threading import Thread
import ctypes
import inspect
...
class Mainpg_fun(Mainpg):
    ...
    def package_bn_f(self):
        try:
            ...
            currentPackage = subprocess.getstatusoutput(
                self.adb + 'shell dumpsys window | findstr mCurrentFocus ')  # 当前包名
            top = tk.Toplevel(self.root)
            top.wm_attributes('-topmost', 1)
            top.title('--包名列表(%s)' % pname)
            top.geometry('%dx%d+%d+%d' % (
            300, self.root.winfo_height(), self.root.winfo_x() + 285, self.root.winfo_y()))  # 设置窗口大小
            t = tk.Text(top)
...
from TkApplication.mods import *
...
from TkApplication.mods import logger_mod
from TkApplication.page import *
